...
class HistoryViewModel extends ChangeNotifier {
  ...
  List<Launch> history = [];
  Duration lastLaunch = Duration(hours: 0);
  ...
  void loadHistory() async {
    try {
      var response = await _api.getHistoryLaunches();
      history.addAll(List<Launch>.from(
    ...
    for (Launch l in history){
      var match = regExp.firstMatch(l.name);
      l.name = match.group(0);
    }

    /*var diff = DateTime.now().difference(history[0].date_local);

    lastLaunch = Duration(hours: diff.inHours);*/
